...
initialShip :: Ship
initialShip = Ship (0,0) 0.0 (0,0) shipPicture 0.0 0.0
...
-- convertShip (Ship _ _ _ outline) =
--     color white (lineLoop outline)

-- The basic design of the ship in Picture format so that Gloss can display it.
shipPicture :: Picture
shipPicture = color white (lineLoop [(0, 0),
                                     (12, - 40),
                                     (10, -35),
                                     (-10, -35),
                                     (-12, -40)])

-- Just extracts the picture from the Ship object. Easier than using pattern matching sometimes.
shipToPicture :: Ship -> Picture
shipToPicture (Ship l a s picture t c) = picture
